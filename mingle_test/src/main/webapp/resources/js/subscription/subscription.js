/* =========================================================
   ...
   실제 결제 연동은 없다. 결제 버튼을 누르면 서버가 결제 내역과
   구독을 함께 만들고, 등급은 기간이 남은 구독으로 계산된다.
   ========================================================= */
...
function pay(product, button) {
    ...
    // 업그레이드면 정가가 아니라 차액을 확인시킨다
    if (!confirm(product.name + "을(를) " + formatPrice(product.payAmount) + "원에 결제할까요?")) {
        return;
    }
